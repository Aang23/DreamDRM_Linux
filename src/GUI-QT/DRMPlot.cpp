...
/* Implementation *************************************************************/
CDRMPlot::CDRMPlot(QWidget *parent, QwtPlot *SuppliedPlot) : SuppliedPlot(SuppliedPlot), DialogPlot(nullptr), bActive(false),
...
void CDRMPlot::OnTimerChart() // TODO make sure timer stopped when receiver not running
{
	...
	case FREQ_SAM_OFFS_HIST:
		...
		/* Prepare graph and set data */
		plot->setAxisTitle(QwtPlot::yLeft, tr("Freq. Offset [Hz] rel. to ") + QString().setNum(pDRMRec->ConvertFrequency(rFreqAcquVal)) + " Hz");
...
void CDRMPlot::SetData(QwtPlotCurve &curve, CVector<_COMPLEX> &veccData)
{
	...
	if (size)
	{
		...
		curve.SETDATA((double *)nullptr, (double *)nullptr, 0);
}
...
void CDRMPlot::PlotDefaults()
{
	...
	main1curve.detach();
	main2curve.detach();
	curve1.SETDATA((double *)nullptr, (double *)nullptr, 0);
	curve2.SETDATA((double *)nullptr, (double *)nullptr, 0);
	curve3.SETDATA((double *)nullptr, (double *)nullptr, 0);
	curve4.SETDATA((double *)nullptr, (double *)nullptr, 0);
	curve5.SETDATA((double *)nullptr, (double *)nullptr, 0);
	hcurvegrid.SETDATA((double *)nullptr, (double *)nullptr, 0);
	vcurvegrid.SETDATA((double *)nullptr, (double *)nullptr, 0);
	main1curve.SETDATA((double *)nullptr, (double *)nullptr, 0);
	main2curve.SETDATA((double *)nullptr, (double *)nullptr, 0);
#if QWT_VERSION < 0x060000
...
void CDRMPlot::SetBWMarker(const _REAL rBWCenter, const _REAL rBWWidth)
{
	...
	if (rBWWidth != (_REAL)0.0)
	{
		dX[0] = pDRMRec->ConvertFrequency(
					(rBWCenter - rBWWidth / 2) * (double)iSigSampleRate) /
				1000.0;
